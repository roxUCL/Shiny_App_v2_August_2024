              ...
              fluidRow(
                sidebarLayout(
                  ...
                  sidebarPanel(width = 3,
                               ...
                               #shiny::selectInput(inputId = "Epic_pipeline_version", label = tags$span("Epic_pipeline_version",tags$i(class = "dropdown",dropdownButton("Info", status = 'success', icon = icon('info'), size = "xs")),sep=""), choices = c("","EPIC_pipeline_v1","EPIC_pipeline_v2"),multiple = F, selected = NULL),
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    { 
      ...
      easyClose = TRUE,
      footer = paste("Estimated job start time:",lubridate::ceiling_date(Sys.time(), "10 minutes"),sep=" ")))
      }else{
        showModal(modalDialog(

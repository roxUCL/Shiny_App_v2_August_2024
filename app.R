...
remove_pipelines = c("Scan_Date","Chip_Type")
version="v2"
...
#This is for personal_computer testing version
...
if(version=="v2"){
  ...
  root_s_drive_dir="/Users/rossellarispoli/Documents/UCL/Projects/Epicv2_analysis/ShinyApp_v2.0/App_latest/output/"
}else{
...
ui <- dashboardPage(
  dashboardHeader(
    title = paste(mytitle,myversion), titleWidth = 450,
              ...
              fluidRow(
                sidebarLayout(
                  ...
                  sidebarPanel(width = 3,
                               ...
                               tags$br(),
                               tags$br(),
                               shiny::selectInput(inputId = "sample_id", label="Sample_ID", choices = c("",isolate(df_completed_job)$Sample_ID),multiple = F, selected = NULL,selectize = T),
                               shiny::selectInput(inputId = "Epic_pipeline_version", label = tags$span("Epic_pipeline_version", bsButton("thisworks", label = "", icon = icon("info"), style = "info", size = "extra-small")), choices = c("","EPIC_pipeline_v1","EPIC_pipeline_v2"),multiple = F, selected = NULL,selectize = T),
                               bsPopover(
                                 id = "thisworks",
                               ...
                               box(id = "view_box",title = "View Report", width = NULL, status = "primary", solidHeader = T,
                                   ...
                                   shiny::selectInput(inputId = "pipelines", label="View Pipeline", choices = "",multiple = F,selectize = F, size=5),
                               ),
...
server <- function(input, output, session) {
  ...
  #rv$date_stamp = format(Sys.time(TZ='GMT'),format="%Y%m%d_%H%M%S")
  rv$date_stamp = format(Sys.time(),tz = "GMT", usetz = TRUE,format="%Y%m%d_%H%M%S")
  ...
  df_job_completed <- reactive({
    flag_completed = input$update_complete_queue
    print("I am reading completed jobs")
    print(completed_collated_jobs)
  ...
  #  shiny::updateSelectInput(session, "job_id", choices =  df_completed_job()$Job_ID[idx], selected =  input$job_id,)
  #})

  
  # Load Sentrix_IDs for matching Sample_IDs
  observe({
    print("I am in 'Load Sentrix_IDs for matching Sample_IDs' section")
    req(credentials()$user_auth)
    idx = which(df_completed_job()$Sample_ID == input$sample_id)
    if (length(idx)>0) {
      shiny::updateSelectInput(session, "sentrix_id", choices = df_completed_job()$Sentrix_ID[idx], selected = df_completed_job()$Sentrix_ID[idx[1]])
      ...
      shiny::updateSelectInput(session, "AV", selected = df_completed_job()$Chip_Type_Path[idx[1]])
      shiny::updateSelectInput(session, "qc_status", choices = df_completed_job()$QC_v1_c2[idx], selected = df_completed_job()$QC_v1_c2[idx[1]])
      
    } else {
      shiny::updateSelectInput(session, "sentrix_id", choices = blank_list, selected = blank_list)
      shiny::updateSelectInput(session, "sentrix_id", choices = blank_list, selected = blank_list)
  ...
  observeEvent(input$info, {
    ...
    shinyalert("Illumina_pipeeline_v2:", "BLABLA", type = "information")
  })
  
  # Load Job_IDs for matching Sample_IDs and Sentrix_IDs
  observe({
    req(credentials()$user_auth & input$sample_id != "" & input$sentrix_id != "")
    idx = which(df_completed_job()$Sample_ID == input$sample_id & df_completed_job()$Sentrix_ID == input$sentrix_id)
    if (length(idx)>0) {
      shiny::updateSelectInput(session, "job_id", choices = df_completed_job()$Job_ID[idx], selected = df_completed_job()$Job_ID[idx[1]])
      shiny::updateSelectInput(session, "scan_date", choices = df_completed_job()$Scan_Date_Path[idx], selected = df_completed_job()$Scan_Date_Path[idx[1]])
      shiny::updateSelectInput(session, "qc_status", choices = df_completed_job()$QC_v1_c2[idx], selected = df_completed_job()$QC_v1_c2[idx[1]])
      shiny::updateSelectInput(session, "AV", choices = df_completed_job()$Chip_Type_Path[idx], selected = df_completed_job()$Chip_Type_Path[idx[1]])
    }
  })
  ...
  observe({
    ...
    print("*********")
    print(pipelines)
    idx = which(df_completed_job()$Sample_ID == input$sample_id & df_completed_job()$Sentrix_ID == input$sentrix_id & df_completed_job()$Job_ID == input$job_id & df_completed_job()$EPIC_pipeline_version == input$Epic_pipeline_version)
    # Check if completed
  ...
  observeEvent(input$pipelines, {
    output$pdfview <- renderUI({
      ...
      print(colnames(df_completed_job()))
      ...
      col_idx = which(colnames(df_completed_job())==paste(p,"_Path",sep=""))
      print(col_idx)
      path_to_report = df_completed_job()[idx,col_idx]
      filename = paste(df_completed_job()$Sample_ID[idx],
                       "_",

...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (input$sample_id !="" & input$Epic_pipeline_version !="" ){
      ...
      shiny::updateSelectInput(session, "scan_date", choices = sel[,14], selected = sel[1,15])
      shiny::updateSelectInput(session, "sentrix_id", choices = sel[,2], selected = sel[1,2])
      shiny::updateSelectInput(session, "job_id", choices =  sel[,3], selected =  sel[1,3],)
      shiny::updateSelectInput(session, "QC_status", choices = sel[,14], selected = sel[1,17])
  ...
  observe({
    ...
    print(input$sentrix_id)
    print(df_completed_job()$sample_ID)
    print(df_completed_job()$Epic_pipeline_version)
    print(df_completed_job()$Sentrix_ID)
    ...
    if (length(idx)>0) {
      print("*$*$*$*$")
      print(df_completed_job()$Scan_Date_Path[idx[1]])
